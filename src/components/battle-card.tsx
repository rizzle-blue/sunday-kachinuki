...
export function BattleCard({ name, dojo, card, compact = false }: Readonly<{ name: string; dojo: string; card: SundayCard; compact?: boolean }>) {
  return (
    <article className={`battle-card rarity-${card.rarity} ${compact ? "compact" : ""}`} style={{ "--card-accent": card.accent } as CSSProperties}>
      ...
      <div className="card-copy">
        ...
        <h2>{name}</h2>
        <p className="codename">{card.codename}</p>
      </div>
