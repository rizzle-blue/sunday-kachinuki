...
import { createDaihyo, finalizeBout, getHostConsole, hostSignIn, recordEvent, startBout, startSession, stopSession, undoEvent } from "@/lib/api";
...
function HostPage() {
  ...
  return <main className="page shell"><header><p className="kicker">Host · recorder · one court</p><h1 className="page-title">Court console</h1><div className="status-strip"><span className={`pill ${consoleState.session.state === "live" ? "live" : ""}`}>{consoleState.session.state}</span><span className="pill">{consoleState.session.readyCount} Ready</span><span className="pill">{consoleState.waitingCount} waiting</span></div><div aria-live="polite">{feedback ? <p className="muted">{feedback}</p> : null}</div></header><div className="host-layout"><div className="stack">{match ? <><MatchBoard match={match} />{bout ? <CourtPad match={match} bout={bout} busy={busy} mutate={mutate} /> : null}{match.state === "tiebreak" && !match.bouts.some((item) => item.position === "daihyo") ? <Daihyo match={match} busy={busy} mutate={mutate} /> : null}</> : <section className="panel"><h2>Chưa có current match</h2><p className="muted">Khi đủ sáu người, bắt đầu formation. Nếu session đã completed, court sẽ giữ trạng thái cuối.</p></section>}</div><aside className="panel host-sidebar"><p className="kicker">Session controls</p><h2>60-minute soft limit</h2><p className="muted">End after current không hủy match đang diễn ra. Sunday không có recovery hoặc reopen.</p><div className="stack">{consoleState.session.state === "lobby" ? <button className="button" disabled={busy || consoleState.session.readyCount < 6} onClick={() => void mutate(startSession,"Formation đã được tạo.")}>Start formation</button> : null}{["live","stopping"].includes(consoleState.session.state) ? <button className="button danger" disabled={busy || consoleState.session.state === "stopping"} onClick={() => void mutate(stopSession,"Session sẽ đóng sau current match.")}>End after current</button> : null}</div></aside></div></main>;
}
